...
app.use((req, res, next) => {
    ...
    // Allow localhost in any environment (including production) for local testing against prod backend
...
export const initPromise = (async () => {
    // 1. Register routes and static files FIRST so the server can respond immediately
    // This prevents "infinite loading" or 404s during Vercel cold starts while DB connects
    ...
    app.get("/api/debug/logs", (_req, res) => {
        debugLogger.add("Log download requested");
        const logs = debugLogger.getLogs();

        res.setHeader('Content-Type', 'text/plain');
        res.setHeader('Content-Disposition', `attachment; filename=server-logs-${Date.now()}.log`);
        res.send(logs);
    });
    ...
    console.log("[STARTUP] Routes & Static files registered");

    // 2. Initialize database pool in BACKGROUND
    // We do NOT await this before routes are ready, to avoid blocking the server response
    (async () => {
        console.log("[STARTUP] Initializing database pool (background)...");
        try {
    ...
    console.log("[STARTUP] Initialization sequence completed (DB init continuing in background)");
})();
