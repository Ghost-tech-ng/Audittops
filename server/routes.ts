...
import { debugLogger } from "./debug-logger"; // Added debug logger
...
export async function registerRoutes(
  ...
  app.post("/api/auth/login", requestTimeoutWrapper(12000), async (req, res) => {
    ...
    try {
      debugLogger.add(`[LOGIN] Attempt started`, { requestId, ip: req.ip });
      ...
      // DEBUG LOG
      debugLogger.add(`[LOGIN] Successful authentication for user: ${user.username} (${user.id})`);
  ...
  app.post("/api/auth/register", async (req, res) => {
    try {
      debugLogger.add(`[REGISTER] Registration attempt started`, { ip: req.ip });
      ...
      debugLogger.add(`[REGISTER] Bootstrapping organization for ${email}`);
      const { organization, user } = await storage.bootstrapOrganizationWithOwner(
        {
      ...
      debugLogger.add(`[REGISTER] Bootstrap complete. User ID: ${user.id}`);
      ...
      debugLogger.error(`[REGISTER] Error during registration`, error);
      res.status(400).json({ error: error.message });
    }
  });
